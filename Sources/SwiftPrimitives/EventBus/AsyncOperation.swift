...
public final class AsyncOperation: Operation, @unchecked Sendable {
    private let activeTask: Synchronized<Task<Void, Never>?> = .init(nil)
    ...
    private let _isExecuting: Synchronized<Bool> = .init(false)
    ...
    private let _isFinished: Synchronized<Bool> = .init(false)
