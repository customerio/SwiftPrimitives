...
struct SynchronizedTests {
    ...
    func testEquals() throws {
        let sync1 = Synchronized(1)
        let sync2 = Synchronized(2)
    ...
    func testComparisons() throws {
        let base = Synchronized(1)
        let greater = Synchronized(2)
        let equal = Synchronized(1)
        let less = Synchronized(0)
    ...
    func testPlusEqualsSynchronizedInteger() throws {
        let initial1 = 31415
        let initial2 = 27182
        let sync1 = Synchronized(initial1)
        let sync2 = Synchronized(initial2)
